 ...
 * 상수시간 비교한다. 시크릿이 빈 문자열(미설정, trim 후에도)이면 헤더 값과 무관하게 무조건
 * 403 — 컨테이너 env에 시크릿을 안 넣은 채 배포돼도 발급 경로가 열리지 않는다(fail-closed, S10).
 *
 * <p>설정값과 헤더 값 모두 {@code trim()} 후 비교한다 — 게이트웨이의 {@code PatExchangeClient}가
 * 자기 쪽 시크릿 사본을 trim해서 보내므로, {@code .env}에 트레일링 스페이스가 섞이면(흔한 실수)
 * trim 없이는 모든 PAT 교환이 이유 없이 503으로 깨진다.
 *
...
public class InternalSecretFilter extends OncePerRequestFilter {
    ...
    /** 생성 시점에 trim해 저장 — 매 요청마다 다시 trim할 이유가 없다. */
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        String trimmedHeader = header == null ? null : header.trim();
        boolean valid = !secret.isEmpty()
                && trimmedHeader != null
                && MessageDigest.isEqual(
                        secret.getBytes(StandardCharsets.UTF_8),
                        trimmedHeader.getBytes(StandardCharsets.UTF_8));
        if (!valid) {
